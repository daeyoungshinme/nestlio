...
- `/schedule` — 일정. 담당자 배분(`assignee_id`, `null`=공동)과 완료 체크가 있는 부부 공동 캘린더/할 일 목록(`Event` 모델, `app/models/event.py`) 전용 화면 — 담당자 필터 탭 + 월간 캘린더 + 캘린더 아래 날짜별 접이식 목록(`ScheduleMonthList`, 오늘 그룹만 기본 펼침) + 날짜 클릭 시 뜨는 모달에서 일정 추가/수정/삭제/완료 체크를 전부 처리한다. 가계부(`/transactions`) 캘린더의 "일정" 탭(`LedgerDayModal`)은 같은 `Event` 데이터를 완료 체크와 함께 훑어볼 수만 있는 읽기 전용에 가까운 미리보기이고("고정지출 납부일 등을 잊지 않게 도와주는 개인 일정·리마인더"), 추가/수정/삭제는 "전체 일정 관리 →" 링크로 이 페이지로 보낸다 — 두 화면이 같은 컴포넌트(`components/schedule/ScheduleEventList` 등)를 재사용하면서도 `readOnly` prop으로 기능 범위를 분리해 CRUD 진입점이 중복되지 않게 한다. 대시보드의 `TodayScheduleCard`도 마찬가지로 오늘 일정 훑어보기+완료 체크+빠른 추가만 제공하고 "전체 일정 보기 →"로 이 페이지로 연결한다. `nav.ts`의 사이드바 그룹에서는 별도 그룹이 아니라 "지출관리"(가계부와 같은 그룹)에 속하며, 모바일 하단탭에서는 상시 노출 4개에 들지 못해 "더보기" 시트에 있다 — 사용 빈도상 자산현황/목표보다 낮게 취급.
...
새 페이지 추가 시 `App.tsx`의 `<Route>`뿐 아니라 `constants/nav.ts`의 `SIDEBAR_NAV_GROUPS`(데스크톱 사이드바, "지출관리"/"자산·목표"/"리포트" 3개 그룹 + 그룹 헤더 없이 상단/하단에 단독 배치되는 대시보드·설정, 총 7개 항목)도 갱신한다. `SIDEBAR_NAV_ITEMS`는 `SIDEBAR_NAV_GROUPS`를 평탄화한 파생 목록이다. 모바일 `BottomNav`는 터치 타겟을 지키기 위해 `BOTTOM_NAV_PRIMARY_ITEMS`(4개: 대시보드/가계부/자산현황/목표)만 상시 노출하고 나머지(`BOTTOM_NAV_MORE_ITEMS`: 일정/연간리포트/설정)는 "더보기" 바텀시트로 접는다 — 예산·고정지출은 관리 화면 자체가 없어지면서 별도 슬롯이 필요 없어졌고, 그 자리에 다음으로 자주 쓰는 자산현황을 승격했다. 예산·고정지출·일정은 독립 탭이 아니라 가계부(`/transactions`) 페이지의 필터·목록/모달로 흡수되거나(예산·고정지출) 사이드바에서 가계부와 같은 그룹으로 묶인다(일정, 화면 자체는 `/schedule`로 독립 — 위 라우트 설명 참고). 두 배열 다 경로(`to`)로 `SIDEBAR_NAV_ITEMS`에서 찾아 파생시키므로(배열 인덱스가 아니라 경로 문자열 목록), 항목을 늘릴 때 `nav.ts`의 그룹 정의와 두 파생 배열의 경로 목록만 고치면 된다.
