...
- `/accounts` — 자산현황 (구 "계좌", 계좌/저축·투자/대출 3탭 평탄 구성 — 저축·투자현황과 대출현황은 목표탭에서 이곳으로 이동했다: 미래 계획·목표치가 아니라 현재 잔액을 조회·기록하는 자산 스냅샷이라는 성격상 대시보드 순자산 카드(`accounts_total`+`savings_total`-`loans_total`=`net_worth`, `GET /net-worth`)와 개념적으로 한 세트이기 때문. 비상금 잔액 입력(`EmergencyFundCard`)도 같은 이유로 목표탭에서 이 페이지 상단(`NetWorthTrendChart` 옆, 3탭에 속하지 않는 가구 전체 스냅샷 영역)으로 옮겨왔다 — 잔액이 부족할 때의 안내는 대시보드 `InvestSurplusCard`에서만 보여주고 여기서 중복 표시하지 않는다)
