...
- `/schedule` — 일정. 담당자 배분(`assignee_id`, `null`=공동)과 완료 체크가 있는 부부 공동 캘린더/할 일 목록(`Event` 모델) 전용 화면 — 담당자 필터 탭 + 월간 캘린더 + 날짜별 접이식 목록(`ScheduleMonthList`) + 날짜 모달에서 CRUD. 헤더에 "새 일정" 버튼과, Google 연동 시 "구글 캘린더" 가져오기 버튼이 있다(구 가계부 "더보기"에서 이동). `?date=YYYY-MM-DD` 파라미터로 진입하면 그 날 모달을 연다(가계부 `LedgerDayModal`이 연결). 대시보드 `TodayScheduleCard`는 오늘 일정 훑어보기+완료+빠른 추가만 제공하고 "전체 일정 보기 →"로 이 페이지로 연결한다. 하단탭에는 없고(가계부 캘린더로 병합 예정) 가계부 헤더의 "일정" 버튼과 데스크톱 사이드바 "바로가기"로 진입한다.
...
새 페이지 추가 시 `App.tsx`의 `<Route>`뿐 아니라 `constants/nav.ts`도 갱신한다. `PRIMARY_NAV_ITEMS`(홈/가계부/계획/목표/자산 5개)는 모바일 `BottomNav`와 데스크톱 사이드바가 공유하는 주 메뉴이고, `SECONDARY_NAV_ITEMS`(일정/연간리포트)는 데스크톱 사이드바 "바로가기"에만 있다(각각 가계부·계획 › 연간으로 병합 예정이라 모바일에서는 해당 화면 안 링크로 진입). "더보기" 시트는 없다. 설정은 탭이 아니라 `Header.tsx`의 톱니 아이콘으로 들어가며, 다크 모드·로그아웃도 설정 화면 한 곳에만 있다(사이드바 중복 제거). 헤더 페이지 타이틀은 `pageTitleFor()`(`PAGE_TITLES` — 주/보조 메뉴 + 설정·카테고리·거래 데이터)가 가장 긴 경로 prefix로 고른다.
