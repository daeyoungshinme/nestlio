...
새 페이지 추가 시 `App.tsx`의 `<Route>`뿐 아니라 `constants/nav.ts`도 갱신한다. `PRIMARY_NAV_ITEMS`(홈/가계부/계획/목표/자산 5개)는 모바일 `BottomNav`와 데스크톱 사이드바가 공유하는 주 메뉴이고, `SECONDARY_NAV_ITEMS`(일정)는 데스크톱 사이드바 "바로가기"에만 있다(가계부로 병합 예정이라 모바일에서는 가계부 헤더 링크로 진입. 연간리포트는 계획 › 연간 › 실적 분석으로 흡수됨). "더보기" 시트는 없다. 설정은 탭이 아니라 `Header.tsx`의 톱니 아이콘으로 들어가며, 다크 모드·로그아웃도 설정 화면 한 곳에만 있다(사이드바 중복 제거). 헤더 페이지 타이틀은 `pageTitleFor()`(`PAGE_TITLES` — 주/보조 메뉴 + 설정·카테고리·거래 데이터)가 가장 긴 경로 prefix로 고른다.
