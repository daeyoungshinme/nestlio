  ...
  return (
    <div className="space-y-6">
      ...
      {data.by_user.length > 0 && (
        <div className="card">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">이번 기간 함께 모은 돈</h3>
          <p className="text-lg font-bold text-gray-900 dark:text-gray-50 mb-3">{formatKrw(totalUserSavings)}</p>
          <div className="space-y-3">
            {data.by_user.map((u) => (
              <div key={u.user_id}>
                ...
                <ProgressBar pct={totalUserSavings > 0 ? (Math.max(0, Number(u.savings)) / totalUserSavings) * 100 : 0} />
              </div>
            ))}
          </div>
        </div>
