    ...
    >
      {(data) => {
        ...
        const filteredEvents =
          assigneeTab === ALL_ASSIGNEES_TAB
            ? data.items
            : data.items.filter((e) => (e.assignee?.id ?? null) === assigneeFilterId);
        ...
        return (
          <div className="space-y-6">
            ...
            <MonthCalendarGrid
              yearMonth={yearMonth}
              renderCell={(cell) => (
                <ScheduleDayCell {...cell} events={eventsByDate.get(cell.date) ?? []} onSelect={setSelectedDate} />
              )}
            />
                ...
                <div className="p-6 overflow-y-auto">
                  <ScheduleEventList
                    ...
                    onToggleComplete={(event) =>
                      completeMutation.mutate({ id: event.id, completed: !event.completed_at })
                    }
                  />
                </div>
              </Modal>
            ...
            {formTarget && (
              <Modal onClose={() => setFormTarget(null)} title={formTarget === "new" ? "새 일정" : "일정 수정"}>
                <div className="p-6 overflow-y-auto">
                  <EventForm
                    initialValues={
                      formTarget === "new" ? emptyEventFormValues(createDateHint) : eventToFormValues(formTarget)
                    }
