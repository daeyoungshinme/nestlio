 ...
 * planned_monthly_amount(연동 상품의 월 계획 합 또는 목표 월 저축액)가 suggested_monthly_amount의 90% 이상이면 정상으로 본다 — 반올림
...
export function computeGoalStatus(goal: FinancialGoalOut): ProgressStatus {
  ...
  return Number(goal.planned_monthly_amount) >= requiredPace ? "on_track" : "behind";
}
