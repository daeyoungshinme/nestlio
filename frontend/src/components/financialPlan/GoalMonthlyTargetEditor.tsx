...
interface Props {
  ...
  /** "균등분배로 다시 계산" 버튼을 누르면 이 금액(필요금액 - 현재 저축액)을 남은 달에 고르게
   * 나눠 채운다. */
  distributeAmount?: string;
}

/** 장기 목표(kind="goal") 폼이 쓰는 월별 목표금액 편집기 — 시작월/종료월 사이의 각 달에 대해
 * 금액 입력 행을 보여준다. 기간이 바뀌면 그 기간에 맞춰 행을 재생성하되, 이미 입력해 둔 월의
 * 금액은 그대로 보존한다(FundingSourceChecklist와 같은 목록 UI 톤을 재사용). */
