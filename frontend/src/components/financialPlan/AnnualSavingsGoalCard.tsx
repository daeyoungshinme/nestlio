 ...
 *
 * FinancialGoalsSection 안에서 재무목표 목록 바로 위에 붙는 압축된 상단 스트립으로 쓰인다 —
 * 진행률 요약은 항상 보이고, 목표금액을 직접 고치는 입력 폼은 접이식 뒤에 숨긴다. "재무목표
 * 합계로 채우기" 버튼으로 아래 재무목표 목록의 월 저축금액 합계를 그대로 연간목표에 반영할 수
 * 있어, 두 목표 개념을 입력 한 번으로 이어준다. */
  ...
  return (
    <div className="card space-y-2">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">올해 목표 페이스</h3>
      ...
      >
        <span>목표금액 {editOpen ? "접기" : "수정"}</span>
        <ChevronDown
          size={14}
          className={`shrink-0 transition-transform duration-200 ${editOpen ? "rotate-180" : ""}`}
        />
      </button>
