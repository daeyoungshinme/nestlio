...
- `goal_service.py`: CRUD(`create_goal`/`update_goal`/`delete_goal`/`list_goals`/`get_goal`/`update_monthly_target_achieved`), 챌린지 상태 동기화(`sync_challenge_statuses`, `_apply_challenge_completion`), 연동 관리(`_apply_funding_sources` — 목표 저장은 연동 상품의 월 계획액을 건드리지 않는다: 연동 목표의 월 저축액은 상품 월 계획 합계가 원본), growlio 조회(`fetch_growlio_goal_settings`, `fetch_growlio_insight`), 예외 클래스(`MonthlyTargetNotFoundError`, `DuplicateFundingSourceProductError`)만 남는다.
...
- 할부(`split_item_into_months`)는 남은 달에 target을 나눠 가진 항목 **하나**다(`installment_total`/`installment_total_amount`, 회차는 등록 시 고정한 1회차 달 `installment_start_month`로부터 `installment_no_for(ym)`으로 계산 — `start_month`는 달 삭제/앞 달 추가로 움직이므로 회차 기준이 아니다). 한 해를 넘기면 `ValueError` — 라우터가 그 해 남은 달 수만 넘긴다.
