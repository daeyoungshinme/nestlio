...
- 시드 사용자는 spouse1/spouse2 두 명 고정이며 자체 회원가입 기능은 없다 (`scripts/seed_data.py`로 시드). 첫 인증된 요청 시 Supabase 사용자를 로컬 `User` 행으로 미러링한다 (`app/dependencies.py`의 `get_current_user`).
...
- 로컬 백엔드 실행: `run.bat` (내부적으로 `.venv`의 uvicorn을 8899 포트로 실행)
- 로컬 프론트엔드 실행: `cd frontend && npm run dev` (Vite, 5273 포트, `/api` 요청을 8899로 프록시)
- 프론트 개발(소스 수정 즉시 반영, HMR): `dev.sh` (Windows: `dev.bat`) — 백엔드(uvicorn `--reload`)와 프론트(Vite dev 서버)를 동시에 띄운다. `http://localhost:5273`으로 접속하면 프론트/백엔드 코드 수정이 재빌드·재기동 없이 바로 반영된다. `run.sh`/`run.bat`은 `frontend/dist`를 정적 빌드해 서빙하는 방식이라 프론트 수정 시 재빌드가 필요하다 (배포 스냅샷 실행용).
