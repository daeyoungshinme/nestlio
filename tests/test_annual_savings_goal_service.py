...
def test_upsert_creates_new_goal(seeded_db):
    db = seeded_db["db"]
    goal = annual_savings_goal_service.upsert_goal(db, 2026, Decimal("12000000"), Decimal("1000000"))
...
def test_upsert_updates_existing_year(seeded_db):
    db = seeded_db["db"]
    annual_savings_goal_service.upsert_goal(db, 2026, Decimal("12000000"), Decimal("1000000"))
    updated = annual_savings_goal_service.upsert_goal(db, 2026, Decimal("15000000"), None)
    assert updated.target_amount_krw == Decimal("15000000")
    assert updated.monthly_target_krw is None
    assert len(annual_savings_goal_service.list_goals(db)) == 1


def test_list_goals_orders_by_year_desc(seeded_db):
    db = seeded_db["db"]
    annual_savings_goal_service.upsert_goal(db, 2025, Decimal("10000000"), None)
    annual_savings_goal_service.upsert_goal(db, 2026, Decimal("12000000"), None)
...
def test_compute_progress_for_past_year_uses_all_12_months(seeded_db):
    ...
    goal = annual_savings_goal_service.upsert_goal(db, 2024, Decimal("6000000"), None)
...
def test_compute_progress_for_future_year_is_all_zero(seeded_db):
    db = seeded_db["db"]
    goal = annual_savings_goal_service.upsert_goal(db, 2027, Decimal("12000000"), None)
...
def test_compute_progress_guards_against_zero_target(seeded_db):
    db = seeded_db["db"]
    goal = annual_savings_goal_service.upsert_goal(db, 2026, Decimal("0"), None)
...
def test_compute_progress_divides_target_equally_when_monthly_not_set(seeded_db):
    db, user, salary = seeded_db["db"], seeded_db["user"], seeded_db["salary"]
    goal = annual_savings_goal_service.upsert_goal(db, 2026, Decimal("12000000"), None)  # 균등분배 -> 월 1,000,000
