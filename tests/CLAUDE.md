...
- `test_plan_status.py`(`pct_of`/`status_from_pct` 경계값·폴백), `test_growlio_client.py`(`to_decimal_krw`/`find_by_growlio_id`/`already_linked_growlio_ids`/`sync_linked_rows` 순수 헬퍼) — 여러 곳의 근거라 간접 커버에서 전용 파일로 승격.
